...
vector<Congregate *> Congregate::congregates = {};

int Congregate::searchCongregate(string name, Congregate *&ptr)
{
	for (auto cong : Congregate::congregates)
	{
		if (cong->name == name)
		{
			ptr = cong;
...
Congregate::Congregate(string name, string type, Date startDate, Date endDate)
{
	this->venues = {};
	this->name = name;
	this->type = type;
	...
	Congregate::congregates.push_back(this);
}

int Congregate::reserveVenue(Venue &ven)
{
...
int Congregate::freeVenue(Venue &ven)
{
	// finding venue
	int index = 0;
...
int Congregate::addEvent(Venue &ven, Date date, Time fromHour, Time toHour, string eventName)
{
...
int Congregate::deleteEvent(Venue &ven, Date date, Time fromHour, string eventName)
{
	// checking if venue exists
	bool flag = false;
	for (auto venue : this->venues)
...
int Congregate::showCalendar(Venue &ven)
{
...
int tokenize(vector<string> &arr)
{
	...
	for (int y = 0; y < in.length(); y++)
	{
		...
		{ // if its a space then we need to turn one flag off
			...
			// else if (y > 0 && in[y - 1] == ' ') {
			// 	// if two spaces in a row with all flags off, error
			// 	return -1;
			// }
		}
...
int main(void)
{
	...
	while (1)
	{
		...
		else if (arr[0] == "addCongregation")
		{
			...
			new Congregate(arr[1], arr[2], *startDate, *endDate);
			cout << 0 << endl;
			delete startDate;
			delete endDate;
		}
		else if (arr[0] == "deleteCongregation")
		{
			...
			arr[1].erase(0, 1);
			Congregate *cong;
			if (Congregate::searchCongregate(arr[1], cong) != 1)
			{
				cout << "-1\nError" << endl;
				continue;
			}
			...
			Congregate::congregates.erase(Congregate::congregates.begin() + index);
		...
		else if (arr[0] == "showCongregations")
		{
			...
			cout << Congregate::congregates.size() << endl;
			for (auto cong : Congregate::congregates)
			{
		...
		else if (arr[0] == "reserveVenue")
		{
			...
			Congregate *cong;
			Venue *ven;
			if (Congregate::searchCongregate(arr[3], cong) != 1 || Venue::searchVenue(arr[1], arr[2], ven) != 1 || cong->reserveVenue(*ven) != 1)
			{
				cout << "-1\nError" << endl;
				continue;
			}
		...
		else if (arr[0] == "freeVenue")
		{
			...
			Congregate *cong;
			Venue *ven;
			if (Congregate::searchCongregate(arr[3], cong) != 1 || Venue::searchVenue(arr[1], arr[2], ven) != 1 || cong->freeVenue(*ven) != 1)
			{
				cout << "-1\nError" << endl;
				continue;
			}
		...
		else if (arr[0] == "showReserved")
		{
			...
			Congregate *cong;
			arr[1].erase(0, 1);
			if (Congregate::searchCongregate(arr[1], cong) != 1 || cong->showReserved() != 1)
			{
				cout << "-1\nError" << endl;
				continue;
			}
		...
		else if (arr[0] == "addEvent")
		{
			...
			if (Congregate::searchCongregate(arr[1], cong) != 1 || Venue::searchVenue(arr[2], arr[3], ven) != 1 || checkDate(arr[4], date) != 1 || checkTime(arr[5], start) != 1 || checkTime(arr[6], end) != 1 || cong->addEvent(*ven, *date, *start, *end, arr[7]) != 1)
			{
				cout << "-1\nError" << endl;
				continue;
			}
		...
		else if (arr[0] == "deleteEvent")
		{
			...
			if (Congregate::searchCongregate(arr[1], cong) != 1 || Venue::searchVenue(arr[2], arr[3], ven) != 1 || checkDate(arr[4], date) != 1 || checkTime(arr[5], time) != 1 || cong->deleteEvent(*ven, *date, *time, arr[6]) != 1)
			{
				cout << "-1\nError" << endl;
				continue;
			}
		...
		else if (arr[0] == "showCalendar")
		{
			...
			if (Congregate::searchCongregate(arr[1], cong) != 1 || Venue::searchVenue(arr[2], arr[3], ven) != 1 || cong->showCalendar(*ven) != 1)
			{
				cout << "-1\nError" << endl;
				continue;
			}
