...
public:
    ...
    //Getter functions for the event object as we have private attributes
    int getdate()
...
public:
    ...
    //Getterfunctions for the location object as we have private attributes 
    string getstadium()
    { 
...
public:
    ...
    {   
        ...
        while (curr != NULL)
        {
            if (curr->getdate() == date && curr->getbegin() == begin && curr->geteventname()==eventname)
            {
                ...
                delete curr;//Invokes the destructor to delete
                cout << "0" << endl;
                return;
            }
            prev = curr;
       ...
       cout<<-1<<endl<<"Error"<<endl;//Error message
    }
    ...
    {   
        ...
        int eventcount = 0;//Keeps the  track of number of events
        Event *curr = locations[pos]->getlist();
...
int main()
{   
    using namespace OlympicsScheduler;
    OlympicsScheduler::Scheduler scheduler;//Creating an instance of the scheduler
